...
export default function Card(props: {
	...
	return (
		<Link
			...
			className="relative mx-auto max-w-[80vw] customMd:max-w-none flex w-full min-w-[22rem] basis-1/2 cursor-pointer flex-col overflow-hidden rounded-xl"
		>
			<Image
			...
			<div
				...
				} relative justify-end first-letter flex flex-col rounded-xl px-4 md:px-8 transition-all duration-700`}
			>
				<div
					className={`flex flex-col text-sm ${
						props.primary ? 'gap-y-1 customMd:pt-64' : 'gap-y-1 customMd:pt-20'
					} pt-32 font-lato w-full text-[#F8F8F8]`}
				>
					...
					<div className="flex flex-col items-center gap-y-3 gap-x-5 pt-2 pb-6 lg:pb-8 lg:flex-row">
						<div className="rounded-2xl self-start border border-white px-4 py-1 xl:text-lg text-white xl:px-5 xl:py-2">
							{props.topic}
						</div>
						{props.createdDate && props.author && (
							<section className="flex flex-col self-start gap-y-1 xl:text-lg">
